  ...
  (type-case ExprC e
    ...
    [(ifC a b c)
     (let ([v1 (interp-helper a env)])
       (cond [(not (boolV? v1))
              (error 'interp-helper "no es un valor booleano")]
             [(boolV-value v1)
              (interp-helper b env)]
             [else
              (interp-helper c env)]))]
    ...
    [(appC func arg)
     (let ([f (interp-helper func env)])
       (if (funV? f)
           (let ([newEnv (extend-env (funV-param f) (interp-helper arg env) (funV-env f))])
             (interp-helper (funV-body f) newEnv))
     ...
     (if (numV? left)
         (if (numV? right)
             (numV (+ (numV-value left)
                      (numV-value right)))
             (error 'binop "argumento incorrecto: right"))
         (error 'binop "argumento incorrecto: left"))]
    [(appendO)
     (if (strV? left)
         (if (strV? right)
